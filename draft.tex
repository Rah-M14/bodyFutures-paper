...
\textbf{(a)}~Without body-awareness, PPO-Task depletes Future Action Capacity (FAC) as locomotion overloads the robot's physiology, ending in body collapse. PPO-IWM maintains high FAC throughout by learning torque-conservative gaits.
\textbf{(b)}~The IWM self-preservation loop: body observations feed a multi-horizon predictor ($h\!=\!1,5,25,100$ steps) that issues a shaped reward proportional to predicted FAC loss. The Interoceptive Anticipation Horizon (IAH\,=\,379 steps) marks the advance warning before collapse - a forward signal absent from all reactive and recurrent baselines.
\textbf{(c)}~Evidence: PPO-IWM simultaneously achieves $+10.7\%$ task reward, $+10.4\%$ final body health, and $0\%$ episode collapse versus PPO-Task ($\star$\,$p\!<\!0.05$, 5 seeds, 50 episodes).}
\label{fig:story}   
\end{figure*}
