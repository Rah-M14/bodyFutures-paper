...
\textbf{Scope conditions.} The observability criterion (Q3, Table~\ref{tab:summary}) is the primary scope condition; full per-morphology scope analysis can be found in the Appendix~\ref{app:scope}.

\section{Conclusion}
\label{sec:conclusion}

A robot that models the future of its own body learns efficiency not because efficiency is rewarded, but because overexertion destroys its future viability, this is the central finding.
The traditional safe-RL tradeoff between task performance and body preservation weakens when agents predict degradation rather than react to it: FAC quantifies what is at stake, IWM provides the forward signal, and IAH makes impending incapacitation auditable before failure occurs.
BodyFutures-Bench and IAH are released as infrastructure for this shift; the next step is real hardware, where self-preservation is no longer a simulated regularizer but a physical necessity imposed by the body itself.
