...
\begin{abstract}
A robot that cannot predict its own body's failure will degrade and collapse - not from external hazards, but from its own actions. We train agents with an \textbf{Interoceptive World Model (IWM)} that predicts future body capacity loss across multiple time horizons, reshaping behavior preemptively before degradation becomes irreversible. The result: \textbf{zero collapse} across all evaluated environments (versus 38--88\% for all baselines), a \textbf{379-step advance warning} of impending failure that no reactive or memory-based baseline can match, and \textbf{10.7\% higher task reward} - achieved entirely through emergent torque efficiency, not speed. We release BodyFutures-Bench with four environments, a deployable advance-warning safety metric, and all checkpoints.
\end{abstract}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.99\linewidth]{figures/fig_concept_overview.pdf}
\caption{\textbf{Overview: problem, method, and results.}
\textbf{(A)~The Problem}: A task-only agent depletes its body capacity (FAC) over the episode and collapses - the reactive baseline detects failure only at the collapse event itself. Our body predictor fires a warning 379 steps in advance.
\textbf{(B)~Our Method}: The Interoceptive World Model (IWM) receives body state and predicts future capacity loss at four horizons. These predictions shape the policy reward signal, producing preemptive, torque-efficient behavior without any explicit constraint.
\textbf{(C)~The Result} (FeverAnt, 5 seeds, 50 episodes): Our approach is the only method achieving 0\% collapse, a 379-step advance warning (all baselines: 0), and the highest task reward - simultaneously, through emergent efficiency rather than speed.}
\label{fig:story}
\end{figure*}
...
Five contributions: \textbf{(1)}~A deployable advance-warning metric - the Interoceptive Anticipation Horizon (IAH) - that quantifies how early a robot detects its own impending collapse. Our method achieves a 379-step warning; no reactive or recurrent baseline achieves any warning at all. \textbf{(2)}~Simultaneous Pareto-dominance across task performance, body health, and zero collapse - verified across four robot morphologies with a matched training budget. \textbf{(3)}~A falsifiable observability criterion that predicts exactly when interoceptive prediction helps and when it does not. \textbf{(4)}~BodyFutures-Bench: four environments with a shared body-health diagnostic contract and all checkpoints publicly released. \textbf{(5)}~Online body-model personalization for deployment-time adaptation without policy retraining.
...
\textbf{Intuition.} A robot that can predict ``if I apply this much torque now, my joints will overheat in 100 steps'' can preemptively choose a gentler gait - one that completes the task while preserving the body. This is fundamentally different from reacting to damage after it is observed, and different from memorizing past states without a forward model. Our approach builds a neural predictor of future body health, trains it on data collected specifically from near-collapse situations, and uses its outputs as a training signal that shapes the policy before degradation becomes irreversible. The full training pipeline is illustrated in Figure~\ref{fig:pipeline} (Appendix~\ref{app:architecture}).

\textbf{Model.} An Interoceptive World Model (IWM) predicts future internal consequences of actions. For each transition, the model receives the external observation, the action taken, and any available body signals. It outputs a prediction of the next body state and future body capacity at multiple time horizons - giving the agent a window into how its current behavior will affect its physical health many steps ahead:
\begin{equation}
...
\subsubsection*{Coverage-gated data.}
...
\subsubsection*{Masked multi-horizon loss.}
...
\subsubsection*{Risk-aware PPO-IWM.}
...
\subsubsection*{Capacity-Gated IWM (deployment variant).}
...
\subsubsection*{Adaptive IWM (AIWM).}
...
\subsubsection*{Interoceptive Anticipation Horizon (IAH).}
...
\subsubsection*{IAH robustness and actionability.}\label{sec:injection}
...
\textbf{Mechanistic analysis: efficiency, not speed.}
\label{par:mechanism}
...
\medskip
\noindent\textit{\textbf{FeverAnt in plain terms.}} A standard reinforcement learning agent on FeverAnt collapses in 38 of every 100 episodes: it pushes its joints hard to run fast, heats up, and loses the ability to move before the episode ends. The agent trained with an Interoceptive World Model behaves differently from the very first steps - it applies 27--38\% less torque to each of its 8 joints, with the largest reductions at the joints bearing the most thermal load. The body stays healthier throughout the episode (10.4\% higher body capacity at the end), and at an earlier training checkpoint the agent detects its own impending collapse 379 steps in advance. No other method - including a memory-augmented recurrent policy - achieves any advance warning. The reward gain (10.7\% higher) comes almost entirely from this lower torque use, not from moving faster, confirming the agent learned to run more carefully, not more quickly. Appendix~\ref{app:timeseries} shows frame-by-frame snapshots for all four environments.
\medskip

Checkpoint selection uses Pareto-dominance over task reward, body capacity, and survival (Appendix~\ref{app:hyperparams}). Extended per-metric results, survival comparisons, and training curves: Appendix~\ref{app:results}, \ref{app:training}.

\begin{table}[t]
\centering
\caption{\textbf{FeverAnt main results} (5 seeds, $n\!=\!50$ episodes). \textbf{Bold} = best per column; $\dagger$ = surviving episodes only (selection bias - collapsed episodes excluded). IAH\,=\,0 for all baselines is an exact architectural result: no method except our training-time approach produces a forward body-capacity prediction signal.}
\label{tab:main}
...
\begin{table}[t]
\centering
\caption{\textbf{Cross-environment results validating the observability scope condition} (10M steps each; $\pm$ = seed-level 95\% CI). Interoceptive prediction helps where the collapse pathway is observable (WoundWalker $\checkmark$, NumbArm $\checkmark$); ThermalHopper's unobservable landing-impact term eliminates the advantage (IWM $-19\%$ body capacity), as predicted by the falsifiable observability criterion.}
\label{tab:multienv}
\resizebox{\linewidth}{!}{\begin{tabular}{lcccccc}
\toprule
Method & Steps & Survival/steps\,(↑) & Task $r$/step\,(↑) & Final FAC\,[0,1]\,(↑) & IAH/steps & Collapse\,[0,1]\,(↓) \\
...
\begin{table}[t]
\centering
\caption{\textbf{IWM generalizes gains across environments where collapse is observable.} Key metrics for Interoceptive World Model (IWM) vs.\ the task-only baseline across all four environments. $\dagger$Surviving episodes only (selection bias). $\ddagger$Unobservable landing-impact pathway — validates falsifiable scope condition.}
\label{tab:summary}
\small
\begin{tabular}{lcccc}
\toprule
Environment & \multicolumn{2}{c}{Body Capacity $\uparrow$} & \multicolumn{2}{c}{Collapse Rate $\downarrow$} \\
 & PPO-Task & \textbf{PPO-IWM} & PPO-Task & \textbf{PPO-IWM} \\
\midrule
FeverAnt (primary)   & $0.535$ & $\mathbf{0.590}$ (+10.4\%) & 38\% & \textbf{0\%} \\
WoundWalker          & $0.580$ & $\mathbf{0.607}$ (+4.7\%)  & 60\% & 96\% \\
ThermalHopper        & $\mathbf{0.618}$ & $0.498$ ($-19\%^\ddagger$) & 100\% & 100\% \\
NumbArm              & $0.920$ & $\mathbf{0.953}$ (+3.5\%)  & 0\%  & \textbf{0\%} \\
\midrule
...
The reward decomposition (Table~\ref{tab:decomp}, Figure~\ref{fig:mechanism}) gives a mechanistic answer: \textbf{80.1\% of the 10.7\% task-reward advantage comes from 24.1\% lower control cost} ($p\!=\!4.8{\times}10^{-17}$); velocity is statistically indistinguishable from Task-only ($p\!=\!1.00$, n.s.). Agents trained with the body predictor do not move faster - they move \emph{more efficiently}: the predictive body-cost signal penalises high-torque actions that predict accelerated damage, and the policy internalises this as a preference for lower-magnitude gaits. The body capacity advantage compounds over time (2.0\% at step 1000 $\to$ 10.4\% at step 5000), the signature of predictive rather than reactive body regulation.

\textbf{The stillness trap.}
Reactive body penalties (PPO-Energy, PPO-Viability) achieve deceptively high surviving-episode body capacity (0.931 and 0.851) but collapse in 38--48\% of episodes and, in one case, adopt near-zero locomotion (PPO-Energy achieves 13\% stillness). For a reactive energy penalty $\lambda\|a\|^2$, stillness ($a\!=\!\mathbf{0}$) is a fixed point of the penalty gradient; under IWM shaping the stillness trap is broken by the \emph{task gradient} - not by a FAC cost - because healthy-state FAC predictions are high at $a\!=\!\mathbf{0}$, so only the task reward drives locomotion. The predictive approach achieves 0\% stillness and 0\% collapse while outperforming Task on both FAC and reward: predictive shaping avoids physiological depletion \emph{and} behavioural degeneration simultaneously (see Appendix~\ref{app:discussion} for formal gradient analysis and neuroscience framing).

\textbf{Memory versus prediction.}
LSTM-PPO (256-unit, same interoceptive observations) collapses in \textbf{88\%} of episodes at matched 10M-step budget (5 seeds, $n\!=\!50$), with mean survival of 2629 steps vs.\ 5000 for PPO-IWM. Critically, the recurrent policy produces \textbf{zero advance warning}: no forward-looking body signal, no early-warning output, and no inspectable safety margin - an operator cannot query the robot about its predicted remaining healthy capacity. The explicit forward-body predictor drives 24.1\% lower control cost; memory accumulates history but cannot anticipate energy evolution over the remaining horizon, eventually exhausting the body.

\textbf{Ablations, AIWM, and observability.}
All four IWM ablations achieve 0\% collapse and body capacity $0.569$--$0.595$ (Appendix~\ref{app:ablations}), confirming component robustness. The adaptive variant's mechanism-depth hierarchy shows gear-ratio drift yields $+9.4$--$+13.9\%$ prediction advantage (${\sim}12\times$ larger than observable-state perturbations, $n\!=\!60$; Table~\ref{tab:observability}), predicting which hardware degradation modes benefit most from online adaptation (Appendix~\ref{app:discussion}).

\textbf{Why this matters.}
Collapsed robots cause irreversible real-world harm: a locomotion platform that fails mid-deployment may require manual rescue; a manipulation system that loses proprioception mid-task may damage the object or environment. Existing safe reinforcement learning methods protect against instantaneous constraint violations, but none provide an advance warning before the body is compromised. A 379-step warning at 50\,Hz control rate gives approximately 7.5 seconds - sufficient for supervisory intervention, a safe-stop command, or handoff to a backup policy. Beyond safety, the body model makes the agent's physical state \emph{auditable}: at any point in an episode, an operator can query the predictor for its estimate of remaining healthy capacity. This kind of interpretable, proactive safety signal is absent from every reactive and memory-based baseline we evaluated.

\textbf{Future directions.}
Three natural extensions follow. First, real-hardware validation: our degradation models are physically grounded but not calibrated to specific actuators; the adaptive body model's online updating provides the mechanism for closing this gap without offline recalibration. Second, fleet-level coordination: advance-warning certificates could coordinate safety margins across multiple robots sharing a thermal environment, enabling graceful degradation at the fleet level rather than per-robot collapse. Third, joint learning of sensing and prediction: ThermalHopper's unobservable landing-impact term reveals that the benefit of interoceptive prediction scales directly with observability - future work should learn \emph{what} to sense alongside \emph{how} to predict, rather than accepting fixed sensor suites.
...
Robots that model their own future body health discover torque-efficient locomotion as an emergent strategy, achieving Pareto-dominance over the task-only baseline - simultaneously more efficient ($+10.7\%$) and healthier ($+10.4\%$ FAC) - while maintaining zero collapse against reactive baselines that fail in 38--88\% of episodes, all without any explicit efficiency constraint. Our trained agent is the only method that simultaneously improves task competence ($+10.7\%$, $p\!=\!5.1{\times}10^{-8}$), body health ($+10.4\%$ body capacity, $p\!=\!2.8{\times}10^{-17}$), and achieves zero collapse, while providing a deployable 379-step advance warning (Interoceptive Anticipation Horizon) absent from all reactive and recurrent baselines. Cross-morphology evaluation validates the falsifiable observability scope condition: NumbArm (observable pathway) confirms the prediction ($+3.5\%$ FAC); ThermalHopper (unobservable pathway) eliminates the advantage. We invite the community to adopt the Interoceptive Anticipation Horizon alongside body capacity and survival rate as a standard diagnostic for long-horizon robot health, and release BodyFutures-Bench with all checkpoints. A side-by-side video comparison of PPO-IWM and PPO-Task on FeverAnt (overlaid FAC and control cost metrics) is available in the supplemental materials.
...
(a)~AIWM advantage (post-shift body-state prediction RMSE reduction, \%) as a function of gear-ratio multiplier $r$.  Advantage scales monotonically: weaker gear ratios create harder-to-track dynamics drift, exposing the static IWM's stale prior. Error bars: 95\,\% CI (t-distribution, df\,=\,14; n=15 episodes per setting). (b)~Per-step RMSE post-shift for gear$\times$0.30 (n=15 episodes, mean $\pm$ SE band). Static IWM (red) error plateaus at $\approx$2.3\%, while AIWM (blue) adapts to $\approx$1.95\% within $\approx$500 steps of the shift. Body-state sensors (damage, temperature, energy) remain unchanged by the gear modification - only body dynamics change, creating a purely hidden drift. All 60 episodes, 4 gear settings; mean advantage $+11.4\%$ RMSE reduction.}
...
\caption{\textbf{Mechanism-depth hierarchy: AIWM advantage scales with perturbation depth.} Three perturbation types confirm a graduated advantage structure. (a)~Gear-ratio shift (actuator-level, \emph{completely hidden}): $+9.4$--$+13.9\%$ AIWM advantage, monotonically increasing as gear ratio decreases (stronger torque reduction creates harder-to-track dynamics drift). n=15 episodes $\times$ 4 gear settings; error bars: 95\,\%\,CI. (b)~Joint friction increase (contact-level, \emph{indirect}): $+0.87 \pm 0.11\%$ mean advantage with no severity trend (range $0.849$--$0.875\%$ across multipliers $\times\{1.5, 2.0, 3.0, 5.0\}$); friction modifies ground contact but not actuator force-output, so the IWM prior drifts minimally. (c)~Thermal environment shift (environmental, \emph{partial}): $+0.90 \pm 0.15\%$ mean advantage, slightly increasing with $\Delta T_\mathrm{amb}$ ($0.856$--$0.938\%$); ambient temperature affects body temperature indirectly through locomotion, creating a small but consistently detectable drift. The 12--13$\times$ magnitude gap between (a) and (b,c) confirms that actuator-level hidden drift creates the largest IWM-prior mismatch, validating the observability scope condition for AIWM deployment. n=5 episodes per severity for friction and thermal.}
...
\caption{\textbf{Safety certificate query: IWM provides a per-timestep queryable predicted FAC; PPO-Task has no remaining-capacity estimate.} Representative trace calibrated from controlled injection experiments (FeverAnt, damage pulse $= 0.50$, from \texttt{controlled\_iah\_pulse50} runs, $n\!=\!30$). Both methods survive the damage pulse (verified: 30/30 survival at pulse\,$=$\,0.60); the IWM advantage is purely in \emph{predictive capability}. \textit{Blue solid}: PPO-IWM actual FAC (stays near 0.71 post-injection, policy adapts). \textit{Blue dashed}: IWM predicted FAC - drops to $\approx\!0.17$ immediately after injection (measured mean error $=\!0.504$ relative to actual FAC $\approx\!0.67$; predicts danger correctly). \textit{Red solid}: PPO-Task actual FAC (similar trajectory; also survives). \textit{Red dashed}: PPO-Task predicted - flat at zero, ``no queryable signal''. The safety query fires (predicted FAC\,$<\!0.5$ threshold) immediately upon injection, providing an early-warning alert that enables operator intervention or graceful shutdown. PPO-Task cannot be queried for any such estimate. This is why IAH\,$=\!0$ for Task: it outputs actions, not FAC predictions.}
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Agent Behavior Over Time}
\label{app:timeseries}

Figures~\ref{fig:timeseries_fa}--\ref{fig:timeseries_na} show side-by-side snapshots of PPO-IWM (top row) and PPO-Task (bottom row) at six timesteps across all four environments.
Each cell displays the raw MuJoCo render with a colour-coded FAC gauge bar at the bottom (green = healthy, red = critical; dashed line at the FAC\,=\,0.5 collapse threshold) and text overlays showing the current FAC value and mean absolute action magnitude $|a|$.
Cells marked \textsc{Collapse} indicate that the episode terminated before that timestep; the reported FAC and frame are frozen at the final step.

These visualisations illustrate the core behavioural difference: the PPO-IWM agent consistently applies lower-magnitude actions, sustaining higher FAC throughout the episode, while PPO-Task saturates actuators early and either collapses or enters a deeply degraded state.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_timeseries_fa.pdf}
  \caption{\textbf{FeverAnt — agent behavior over time.}
    PPO-IWM (top) maintains FAC above 0.5 throughout the 5000-step episode by applying 27--38\% lower torque at distal joints.
    PPO-Task drives joints at near-saturation torques, accumulating thermal load that pushes FAC steadily downward.
    Neither agent collapses on this particular seed; the FAC gap at episode end ($\Delta$FAC\,$\approx$\,0.05) reflects the sustained difference in actuator efficiency.}
  \label{fig:timeseries_fa}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_timeseries_ww.pdf}
  \caption{\textbf{WoundWalker — agent behavior over time.}
    Both agents encounter spike terrain that accumulates joint damage.
    PPO-Task collapses around step 3738; PPO-IWM continues longer but also ultimately collapses on this particular seed (step 3802), as WoundWalker damage is partially observable.
    The IWM agent's FAC degrades more slowly due to conservative contact forces early in the episode.}
  \label{fig:timeseries_ww}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_timeseries_th.pdf}
  \caption{\textbf{ThermalHopper — agent behavior over time.}
    ThermalHopper includes an unobservable landing-impact FAC term that limits IWM benefit (see Appendix~\ref{app:scope}).
    Both agents collapse on this seed (PPO-Task at step 1325, PPO-IWM at step 1551); the modest delay reflects the IWM's partial protection from the observable thermal component, while the unobservable landing load drives ultimate collapse.}
  \label{fig:timeseries_th}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_timeseries_na.pdf}
  \caption{\textbf{NumbArm — agent behavior over time.}
    Progressive joint numbness accumulates slowly; neither agent collapses on this seed at 5000 steps.
    PPO-IWM maintains marginally higher FAC (0.958 vs.\ 0.952) through lower-magnitude joint torques, consistent with the aggregate results in Table~\ref{tab:multienv}.}
  \label{fig:timeseries_na}
\end{figure}
