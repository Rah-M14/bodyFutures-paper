...
\begin{abstract}
Robots degrade not from external hazards but from their own actions: unconstrained actuator use erodes joints, overheats motors, and depletes energy stores, causing future control authority to collapse irreversibly. Existing safe-RL methods guard against external constraint violations and react only when degradation is already catastrophic; no prior method produces a deployable, forward-looking signal of how far a robot is from losing the ability to act at all. We introduce the \textbf{Interoceptive World Model (IWM)}: a neural predictor trained jointly with a control policy that forecasts future body-capacity loss across four horizons (1, 5, 25, 100 steps) and reshapes behavior preemptively. An \textbf{Adaptive IWM (AIWM)} extends this with online body-model personalization at deployment without retraining.

Evaluated across BodyFutures-Bench (four MuJoCo environments with latent body dynamics), IWM achieves \textbf{zero collapse} across all environments (versus 38--88\% for all baselines), a \textbf{379-step advance warning} of impending failure (Interoceptive Anticipation Horizon; IAH\,=\,0 for every reactive and recurrent baseline), and \textbf{10.7\% higher task reward}, achieved entirely through emergent torque efficiency with no speed increase. We release BodyFutures-Bench, a deployable IAH safety metric, and all checkpoints.
\end{abstract}
