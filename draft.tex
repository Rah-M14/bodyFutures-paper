...
\small
\begin{tabular}{lp{7.1cm}p{2.8cm}}
\toprule
...
\resizebox{\linewidth}{!}{\begin{tabular}{llcp{6.5cm}}
\toprule
...
\end{tabular}}
\smallskip
\raggedright\footnotesize{The critical condition: AIWM advantage requires that the perturbation changes the body's force-action mapping without immediately appearing in the interoceptive state vector.  Observable-state perturbations (motor damage, actuator weakness, covert wear) yield null results. Gear-ratio shift (MuJoCo \texttt{actuator\_gear} modification) confirms the prediction across all four gear settings: $+9.4\%$ to $+13.9\%$ AIWM advantage (n=60 total; monotonically increasing with torque-reduction severity; mean $+11.4\%$). Joint friction increase and thermal environment shift both yield confirmed but small AIWM advantage ($+0.87 \pm 0.11\%$ and $+0.90 \pm 0.15\%$ respectively; ${\sim}12{\times}$ smaller than gear-ratio shift); friction shows no severity trend ($0.849$-$0.875\%$) while thermal shows a slight upward trend ($0.856$-$0.938\%$) with temperature. This graduated response reveals a \emph{mechanism-depth hierarchy}: \emph{actuator-level} changes disrupt force-output dynamics most directly, yielding the largest AIWM benefit ($+9.4\%$ to $+13.9\%$); \emph{contact-level} and \emph{environmental} changes affect body dynamics more indirectly, yielding smaller but consistently positive adaptation gains ($+0.87\%$ to $+0.90\%$).}
\end{table}
