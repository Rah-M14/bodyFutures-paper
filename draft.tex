...
\newcommand{\methodsec}[1]{\noindent\textcolor{maroonprimary}{\textbf{#1}}\enspace}
...
\begin{abstract}
Robots degrade not from external hazards but from their own actions: unconstrained actuator use
erodes joints, overheats motors, and depletes energy stores, causing future control authority to
collapse irreversibly. Existing safe-RL methods guard against external constraint violations and
react only when degradation is already catastrophic; no prior method produces a deployable,
forward-looking signal of how far a robot is from losing the ability to act at all. We introduce the
\textbf{Interoceptive World Model (IWM)}: a neural predictor trained jointly with a control policy
that forecasts future body-capacity loss across four horizons (1, 5, 25, 100 steps) and reshapes
behavior preemptively. An \textbf{Adaptive IWM (AIWM)} extends this with online body-model
personalization at deployment without retraining.

Evaluated across BodyFutures-Bench (four MuJoCo environments with latent body dynamics), IWM
achieves \textbf{zero collapse} across all environments (versus 38--88\% for all baselines), a
\textbf{379-step advance warning} of impending failure (Interoceptive Anticipation Horizon;
IAH\,=\,0 for every reactive and recurrent baseline), and \textbf{10.7\% higher task reward},
achieved entirely through emergent torque efficiency with no speed increase. We release
BodyFutures-Bench, a deployable IAH safety metric, and all checkpoints.
\end{abstract}
...
\bottomrule
\end{tabular}
\raggedright\footnotesize{$\dagger$~Surviving eps.\ only (selection bias). $\ddagger$~ThermalHopper landing-impact unobservable to IWM; validates scope condition.}
\end{table}
...
All four IWM ablations achieve 0\% collapse (FAC $0.569$--$0.595$; Appendix~\ref{app:ablations}). AIWM gear-ratio adaptation yields $+9.4$--$13.9\%$ RMSE reduction (${\sim}12\times$ larger than observable perturbations, $n\!=\!60$; Appendix~\ref{app:discussion}).
The 379-step IAH at 50\,Hz gives ${\approx}$7.5\,s of supervisory lead time; the IWM also makes the agent's state \emph{auditable} per-step, a proactive signal absent from all reactive and memory-based baselines.
...
\textbf{Scope conditions.} \emph{Falsifiable criterion}: IWM improves health iff the dominant collapse pathway is interoceptively observable, ThermalHopper degrades FAC by $-19\%$ ($p\!=\!2.4{\times}10^{-8}$; unobservable impact); NumbArm improves FAC by $+3.5\%$ ($p\!=\!2.8{\times}10^{-10}$; observable numbness; Appendix~\ref{app:scope}).

\section{Conclusion}
\label{sec:conclusion}

Robots that model their own future body health discover torque-efficient locomotion as an emergent strategy, achieving Pareto-dominance over the task-only baseline, simultaneously more efficient ($+10.7\%$), healthier ($+10.4\%$ FAC), and zero-collapse, all without any explicit efficiency constraint. IWM is the only method that simultaneously improves task competence ($p\!=\!5.1{\times}10^{-8}$), body health ($p\!=\!2.8{\times}10^{-17}$), and achieves zero collapse with a deployable 379-step advance warning absent from all baselines. Cross-morphology evaluation confirms the falsifiable scope condition: NumbArm ($+3.5\%$ FAC); ThermalHopper ($-19\%$ FAC). We release BodyFutures-Bench with all checkpoints and invite adoption of IAH as a standard diagnostic for long-horizon robot health.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.40\linewidth]{figures/snapshot_feverant.png}\hfill
\includegraphics[width=0.18\linewidth]{figures/snapshot_woundwalker.png}\hfill
\includegraphics[width=0.18\linewidth]{figures/snapshot_thermalhopper.png}\hfill
\includegraphics[width=0.18\linewidth]{figures/snapshot_numbarm.png}
...
\noindent Table~\ref{tab:positioning} highlights that IWM is the only method that simultaneously satisfies all five criteria: endogenous degradation, multi-horizon interoceptive prediction, a deployable IAH safety metric, online adaptation, and a dedicated benchmark suite. O'Keeffe et al.\ predict actuator degradation in robot swarms but do not define an anticipation-horizon metric and operate at multi-mission granularity rather than per-timestep intra-episode prediction.
...
The FAC trajectory heat map (Figure~\ref{fig:fac_heatmap_q}) visualises per-episode body health across the full 5000-step horizon. Each row is one episode; each column is one timestep. Colour encodes FAC: green ($\approx$1.0) = healthy, red ($\approx$0) = depleted. Episodes are sorted by final FAC (best to worst, top to bottom). PPO-IWM episodes cluster at the top (high, sustained FAC); PPO-Task episodes show monotone depletion; reactive baselines exhibit sharp mid-episode collapses. The IWM uniqueness is structurally visible: no other method achieves uniformly high FAC rows across all 50 episodes.
...
Colour encodes FAC: green ($\approx$1.0) = healthy, red ($\approx$0) = depleted.
Episodes sorted by final FAC (best $\to$ worst, top to bottom).
PPO-IWM (top cluster): sustained high FAC across all 50 episodes.
PPO-Task (middle): gradual depletion but no collapse.
Reactive baselines (bottom): sharp mid-episode colour transitions mark collapse events.}
\label{fig:fac_heatmap_q}
\end{figure}
