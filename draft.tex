...
\newcommand{\methodsec}[1]{\noindent\textcolor[HTML]{3182CE}{\textbf{#1}}\enspace}
...
A task-only agent depletes its body capacity (FAC) through unconstrained actuator use and collapses — detectable only at the moment of failure.
The Interoceptive World Model (IWM) predicts future FAC loss across four horizons, shapes the policy preemptively, and achieves zero collapse, a 379-step advance warning, and 10.7\% higher task reward — simultaneously — across all evaluated environments.}
\label{fig:story}
\end{figure*}
...
\textbf{Safe RL.} Constrained policy optimization~\citep{achiam2017constrained}, Safety Gym~\citep{ray2019safetygym}, and SafeDreamer~\citep{huang2024safedreamer} address \emph{external} hazards via Lagrangian penalties. BodyFutures differs: the hazard is endogenous — self-induced by the agent's torques — and causes irreversible loss of future control authority, not a one-step constraint violation. None provide a deployable failure-horizon signal; IWM's IAH is absent from all prior safe-RL approaches.

\textbf{World models.} Dreamer~\citep{hafner2020dreamer,hafner2023dreamerv3}, TD-MPC2~\citep{hansen2023tdmpc2}, and MBPO~\citep{janner2019mbpo} predict \emph{external} state transitions. IWM instead predicts \emph{internal} body-state trajectories (future FAC, damage, energy) as a preservation signal, and AIWM updates this predictor online at deployment — no prior world-model work spans all three: internal prediction, online adaptation, and preservation-metric evaluation.

\textbf{Robot self-modeling.} RMA~\citep{kumar2021rma}, teacher-student locomotion~\citep{lee2020learning,miki2022learning}, and fault-detection~\citep{fu2025contrastive,xu2023adapt} estimate \emph{current} body parameters; IWM predicts \emph{future} capacity trajectories and quantifies how far in advance the warning fires (IAH), a temporal gap none of these approaches measure.

\textbf{Privileged information and meta-learning.} Asymmetric actor-critic~\citep{pinto2017asymmetric} and RMA condition on current body parameters; IWM predicts future body trajectories. AIWM uses EWC~\citep{kirkpatrick2017overcoming} to personalise interoceptive predictions to the deployed robot without downtime. See Appendix~\ref{app:relatedwork} for extended comparisons.
...
\methodsec{IAH robustness and actionability.}\label{sec:injection}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.99\linewidth]{figures/Methods.png}
...
\end{tabular}}
\smallskip
\raggedright\footnotesize{$\dagger$ Surviving episodes only (Energy: 26/50, Viability: 31/50) — not comparable to IWM/Task (100\% survival). $\|$ LSTM: 88\% collapse, IAH\,=\,0 (no forward body-prediction signal; memory alone does not suffice). $\S$ CG-IWM: deployment-time governor on frozen PPO-Task (seeds 0--2, $n\!=\!30$, $\tau\!=\!0.10$); no additional training; IAH undefined (gate prevents collapse). IAH\,=\,0 for all baselines is exact: no reactive or recurrent method produces a queryable forward body-capacity signal.}
\end{table}
...
\end{tabular}
\smallskip
\raggedright\footnotesize{$\dagger$ Surviving episodes only — FAC measured on non-collapsed runs; Energy/Viability collapse in 38--48\% of episodes so these figures carry selection bias. $\ddagger$ ThermalHopper's landing-impact FAC term is unobservable to the IWM (not in body-state vector), eliminating the advantage and validating the falsifiable scope condition.}
\end{table}
...
All four IWM ablations achieve 0\% collapse and FAC $0.569$ to $0.595$ (Appendix~\ref{app:ablations}), confirming component robustness. The adaptive variant's mechanism-depth hierarchy shows gear-ratio drift yields $+9.4\%$ to $+13.9\%$ prediction advantage (${\sim}12\times$ larger than observable-state perturbations, $n\!=\!60$; Table~\ref{tab:observability}), predicting which hardware degradation modes benefit most from online adaptation (Appendix~\ref{app:discussion}).
...
Gear-ratio shift (MuJoCo \texttt{actuator\_gear} modification) confirms the prediction across all four gear settings: $+9.4\%$ to $+13.9\%$ AIWM advantage (n=60 total; monotonically increasing with torque-reduction severity; mean $+11.4\%$).
...
The full causal chain from IWM prediction to task-reward advantage is: IWM predicts future FAC loss from current actions (RMSE\,$=\!0.0065$) $\rightarrow$ shaped reward penalises high-torque actions $\rightarrow$ policy learns 24.1\% lower control cost ($0.219$ vs $0.288$\,/step) $\rightarrow$ final FAC is 10.4\% higher ($0.590$ vs $0.535$) $\rightarrow$ task reward is 10.7\% higher ($0.901$ vs $0.814$\,/step). This is emergent efficiency, not a programmed constraint: 80.1\% of the advantage traces back to lower torque, not higher speed.
