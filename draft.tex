...
Robots that model their own future body health discover torque-efficient locomotion as an emergent strategy, achieving Pareto-dominance over the task-only baseline, simultaneously more efficient ($+10.7\%$), healthier ($+10.4\%$ FAC), and zero-collapse, all without any explicit efficiency constraint. We have proposed IWM as a method that simultaneously improves task competence ($p\!=\!5.1{\times}10^{-8}$), body health ($p\!=\!2.8{\times}10^{-17}$), and achieves zero collapse with a deployable 379-step advance warning absent from all baselines. Cross-morphology evaluation confirms the falsifiable scope condition: NumbArm ($+3.5\%$ FAC); ThermalHopper ($-19\%$ FAC). We've released BodyFutures-Bench with all checkpoints and are inviting IAH to become the standard diagnostic for long-horizon robot health.
